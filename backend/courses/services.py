...
def update_lesson_completion(user, lesson):
    ...
    all_modules_completed = all(
        ModuleProgress.objects.filter(enrollment__user=user, module=m, completed_at__isnull=False).exists()
...
def get_next_step(user, course, current_lesson_id=None, current_assessment_id=None):
    ...
    assessments = list(LessonAssessment.objects.filter(
    ...
    if current_index + 1 < len(sequence):
        ...
        if next_type == "lesson":
            ...
            lesson_assessment_session = start_lesson_assessment(user, next_obj.lesson.id)
            return {
                "type": next_type,
                "id": next_obj.id,
                "title": str(next_obj),
