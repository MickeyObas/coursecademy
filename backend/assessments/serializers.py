...
class QuestionSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if (q_type == "FIB") and not data.get('correct_answer'):
            raise serializers.ValidationError("FIB questions must have 'correct_answer' set.")
        
        if (q_type != "FIB") and data.get('correct_answer'):
            raise serializers.ValidationError("Only FIB questions should have a direct 'correct_answer' value set.")
        
        if q_type != "TF" and 'is_true' in data:
            raise serializers.ValidationError("Only TF questions should have the 'is_true' flag set")
        
        if q_type == "TF" and 'is_true' not in data:
            raise serializers.ValidationError("TF questions should have the 'is_true' flag set")
    ...
    def create(self, validated_data):
        ...
        try:
            ...
            raise serializers.ValidationError({'error': f'Invalid assessment type "{content_type_model}"'})

        # Create options later
